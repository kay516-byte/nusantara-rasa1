...
<body class="bg-gray-100 py-10">
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <a href="{{ route('resep.index') }}" class="inline-block m-4 text-orange-600 hover:underline text-sm">← Kembali</a>

        <div class="px-8 pb-4">
            <p class="text-orange-500 text-xs font-semibold tracking-widest uppercase mb-1">Resep Nusantara</p>
            <h1 class="text-4xl font-bold text-gray-800 mb-2">{{ $resep->nama_resep }}</h1>
            <p class="text-gray-500 text-sm">Cita rasa khas Indonesia, mudah dibuat di rumah.</p>
        </div>

        @if ($resep->foto)
            <img src="{{ asset('storage/' . $resep->foto) }}" class="w-full h-72 object-cover">
        @endif
                ...
                <ol class="mt-3 space-y-4">
                    ...
                    @foreach ($steps as $index => $step)
                        ...
                        <li class="flex gap-3">
                            ...
                            <p class="text-sm text-gray-600 pt-0.5">{{ $clean }}</p>
                        </li>
                    @endforeach
                </ol>
            </div>
